...
| `CURSOR_BRIDGE_REQUESTS_LOG` | `~/.cursor-api-proxy/requests.jsonl` | Structured request log: **one JSON object per line** with status, duration, model, engine, account, latency spans, error code, retry/failover count and prompt/completion character counts. Never contains prompts, completions or keys; a sticky conversation id is stored only as a short hash. The plain-text sessions log above keeps working unchanged. |
| `CURSOR_BRIDGE_REQUESTS_LOG_ENABLED` | `true` | Set `false` to skip the JSONL log (the dashboard then falls back to parsing `sessions.log`). |
| `CURSOR_BRIDGE_REQUESTS_LOG_MAX_BYTES` | `33554432` (32 MB) | Rotate the JSONL log to `<path>.1` once it would exceed this size. `0` disables rotation. |
| `CURSOR_BRIDGE_METRICS_ENABLED` | `true` | Serve `GET /metrics` in Prometheus text format (requests, duration and latency-span histograms, admission gauges, account state, failover and rate-limit counters, build info). Set `false` to return **404**. Access is gated like the dashboard's sensitive reads: `Authorization: Bearer <CURSOR_BRIDGE_API_KEY>` when that key is set, otherwise loopback clients only. |
...
| `CURSOR_BRIDGE_MAX_CONCURRENT_RUNS` | `16` | Max concurrent **ACP/CLI** agent runs across the whole proxy. Extra requests wait up to `CURSOR_BRIDGE_ADMISSION_WAIT_MS`, then get **503** with `Retry-After`. |
| `CURSOR_BRIDGE_MAX_CONCURRENT_RUNS_PER_ACCOUNT` | `2` | Max concurrent **ACP/CLI** runs **per account** dir. Usual bottleneck with few accounts — raise to `3`–`4` only if you have RAM headroom; prefer more accounts over a high per-account cap. |
| `CURSOR_BRIDGE_MAX_CONCURRENT_RUNS_SDK` | `48` | Max concurrent **in-process SDK** runs (separate plane from ACP; much lighter on RAM). |
| `CURSOR_BRIDGE_MAX_CONCURRENT_RUNS_PER_ACCOUNT_SDK` | `12` | Max concurrent SDK runs **per account**. Still bounded to protect Cursor rate limits and the Node event loop — not unlimited. |
...
- Relative paths such as `CURSOR_BRIDGE_WORKSPACE`, `CURSOR_BRIDGE_SESSIONS_LOG`, `CURSOR_BRIDGE_REQUESTS_LOG`, `CURSOR_BRIDGE_TLS_CERT`, and `CURSOR_BRIDGE_TLS_KEY` are resolved from the current working directory.
- **Metrics cardinality:** `model` and `account` labels keep at most 40 distinct values each (further values collapse to `other`), and unknown routes collapse to `other`, so scraping stays bounded no matter what clients send. Dashboard polls and `/metrics` scrapes are excluded from request counters and from both request logs.
- **Admission tuning:** ACP/CLI defaults (`16` global / `2` per account) assume heavy children. SDK uses a **separate** plane with higher defaults (`48` / `12`) because runs are in-process. If parallel prompts hit **503**, raise the matching `*_PER_ACCOUNT*` env for that engine **or** add accounts (spreads Cursor rate limits). Do not set SDK caps to “unlimited” — rate limits and event-loop contention still apply. `GET /healthz` exposes both planes (`admission.acp` / `admission.sdk`).
